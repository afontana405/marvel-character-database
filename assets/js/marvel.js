...
function searchMarvelCharacter(query) {
    ...
    fetch(`https://gateway.marvel.com:443/v1/public/characters?nameStartsWith=${query}&apikey=${publicKey}&hash=${hash}&ts=${ts}`) // fetchs the characters from Marvel API
    .then(response => {
        if (!response.ok) {
...
function displayComics(characterId) {
    ...
    fetch(`https://gateway.marvel.com:443/v1/public/characters/${characterId}/comics?apikey=${publicKey}&hash=${hash}&ts=${ts}`) //fetches the comics from MARVEL API
    .then(response => {
        if (!response.ok) {
            throw new Error('Network response was not ok' + response.statusText);
        }
